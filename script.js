/**
 * ============================================================================
 * 🌟 MAPLE OMNI - 코어 실행 스크립트 (script.js)
...
window.addEventListener('popstate', (event) => {
    if (event.state && event.state.page) {
        ...
        else if (event.state.page === 'builder') { if(typeof openOmniBuilder === 'function') openOmniBuilder(); }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (lastPage === 'hunt') { window.startApp(true); history.replaceState({ page: 'hunt' }, "", "#hunt"); } 
    else if (lastPage === 'search') {
        window.openSearchPage(true); history.replaceState({ page: 'search' }, "", "#search");
        ...
        if (lastSearch) {
            ...
            if (cachedData && typeof renderSearchDetail === 'function') renderSearchDetail(cachedData.basic, cachedData.stat, cachedData.item, cachedData.ability, cachedData.symbol, cachedData.dojang);
        }
    } else { window.backToPortal(true); history.replaceState({ page: 'portal' }, "", "#portal"); }
});
