/**
 * ============================================================================
 * 🎨 MAPLE OMNI - UI 및 상호작용 전용 모듈 (ui.js)
 * 탭 전환, 즐겨찾기, 인게임 툴팁, 장비/어빌리티 프리셋 화면 전환 등
 * 화면에 직접적으로 보이는 디자인적 요소들을 담당합니다.
 * ============================================================================
 */
...
function generateInGameTooltipHtml(item, slotName) {
    ...
    if (!noStarParts.includes(part) && !isEventRing) {
        ...
        for (let i = 1; i <= maxStar; i++) { starsHtmlContent += (i <= starCount) ? "★" : "☆"; if (i % 5 === 0) starsHtmlContent += " "; if (i === 15) starsHtmlContent += "<br>"; }
...
function showTooltip(slotName, presetNum) {
    ...
    if (!item) return;
    let tt = getOrCreateTooltip(); tt.innerHTML = generateInGameTooltipHtml(item, slotName); tt.style.display = 'block';
}
...
function switchItemPreset(num) {
    document.querySelectorAll('#itemPresetBtns .preset-btn').forEach((btn, i) => { btn.classList.toggle('active', i + 1 === num); });
    ...
    if (!slotBox) { const powerNode = document.getElementById('res_power'); if (powerNode) { const parent = powerNode.parentElement; parent.innerHTML = ''; slotBox = document.createElement('div'); slotBox.id = 'res_equip_slot_grid'; parent.appendChild(slotBox); } }
    if (slotBox) { slotBox.style.cssText = "display:grid !important; grid-template-columns:repeat(5, 1fr) !important; gap:8px; padding:15px; background:#f8fafc; border-radius:12px; border:1px solid #edf2f7; width: 100%; box-sizing: border-box; margin-bottom: 20px;"; }
    ...
    const shortPot = (txt) => { if (!txt) return ""; return txt.replace("보스 몬스터 공격 시 데미지", "보공").replace("보스 몬스터 데미지", "보공").replace("몬스터 방어율 무시", "방무").replace("크리티컬 데미지", "크뎀").replace("아이템 드롭률", "아획").replace("메소 획득량", "메획").replace("최대 HP", "HP").replace(" : ", ":"); };
    ...
    if (slotBox && equipList) {
        ...
        slotOrder.forEach(sName => {
            ...
            if (item) { let bColor = borderGradeColor[item.potential_option_grade] || "#e2e8f0"; gHtml += `<div onmouseenter="showTooltip('${sName}', ${num})" onmousemove="moveTooltip(event)" onmouseleave="hideTooltip()" style="background:white; border-radius:8px; border:1px solid ${bColor}; display:flex; align-items:center; justify-content:center; aspect-ratio:1/1; cursor:pointer; overflow:hidden;"><img src="${item.item_icon}" style="max-width:85%; max-height:85%; object-fit:contain; pointer-events:none;"></div>`; } 
            else { gHtml += `<div style="background:#f1f5f9; border-radius:6px; border:1px dashed #cbd5e1; aspect-ratio:1/1;"></div>`; }
        });
...
function switchAbilityPreset(num) {
    document.querySelectorAll('#abilityPresetBtns .preset-btn').forEach((btn, i) => { btn.style.background = (i + 1 === num) ? '#e2e8f0' : 'transparent'; btn.style.color = (i + 1 === num) ? '#1e293b' : '#94a3b8'; });
    ...
    if (targetPreset && targetPreset.ability_info) {
        html += targetPreset.ability_info.map((a) => {
            let color = "#475569"; if(a.ability_grade === "레전드리") color = "#15803d"; else if(a.ability_grade === "유니크") color = "#b45309"; else if(a.ability_grade === "에픽") color = "#6b21a8"; else if(a.ability_grade === "레어") color = "#0369a1";
    ...
    } else { abiList.innerHTML = '<div style="color: #94a3b8; text-align: center; font-size: 12px; padding: 15px 0;">어빌리티 정보가 없습니다.</div>'; }
}